...
{
    private static List<string> dependenciesList = new List<string>() { "com.unity.xr.interaction.toolkit", "com.unity.inputsystem" };
    ...
    [MenuItem("SmartEye/Installer Dashboard")]
    ...
    private void OnGUI()
    {
        ...
        InstallerProgress(packagesDownloadedCount, 2, "Install Progress");

        UpdateButtons();

        if (packagesDownloadedCount < 2)
            EditorGUILayout.HelpBox("You have to install all basic package before use the smarteye framework", MessageType.Error);
    ...
    void UpdateButtons()
    {
        ...
        EditorGUILayout.BeginHorizontal();
        GUILayout.Label("Input System :", EditorStyles.boldLabel);

        string nameBtn2 = buttonList[1] != "" ? buttonList[1] : "wait...";

        if (GUILayout.Button(nameBtn2))
        {
            InstallPackage(1, "1.6.3");
        }
        EditorGUILayout.EndHorizontal();
        EditorGUILayout.Space(10);
